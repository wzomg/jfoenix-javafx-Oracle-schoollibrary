...
public class ReserveBookController {
    ...
    @FXML
    public void cancelReserveClick() throws ParseException {
        ...
        if(buttonType1.isPresent() && buttonType1.get() == ButtonType.OK) {
            ...
            update = template.update(sql, bookReserve.getQState().equals("等待处理") ? 0 : 2, bookReserve.getBId(), bookReserve.getRId(), Main.sdf.parse(bookReserve.getQSDate()), bookReserve.getQEDate());
            if(update > 0) {
