...
public class ReaderPmsgController {
    ...
    //初始化数据
    public void initData(String rid, String rName, String rSex, String rUnit, String rDate, Integer rNum1, Integer rTotal1, Integer rSates) {
        rname.setText(rName);
        raccount.setText(rid);
        rsex.setText(rSex);
        runit.setText(rUnit);
        regdate.setText(rDate.split(" ")[0]);
        rNum.setText(String.valueOf(rNum1));
        rTotal.setText(String.valueOf(rTotal1));
        rStatus.setText(rSates == 1 ? "挂失" : "正常");
    ...
    public ObservableList<PieChart.Data> getClassBookGraphStatistics() {
        ...
        // GROUP BY 后面要添加所有查询的字段，否则会报错
        String sql = "SELECT COUNT(*) AS classifyNum, TName FROM book, tkind, hreturn WHERE book.BId = hreturn.HBId AND book.BKind = tkind.TId AND hreturn.HRId = ? GROUP BY tkind.TId, TName ORDER BY classifyNum DESC";
