...
public class ReviewReturnController {
    ...
    //验证通过
    @FXML
    public void verifyClick() throws ParseException {
        ...
        if(bookItem.getNState().equals("申请借阅")) {
            ...
            update = template.update(sql, bookItem.getBId(), bookItem.getRId(), Main.sdf.parse(startTime) , Main.sdf.parse(endTime));
            if(update > 0) {
            ...
            // 此处采用行级触发器，当向已归还表中添加记录后，就触发下面的3个操作
            ...
            update1 = template.update(sql1, bookItem.getBId(), bookItem.getRId(), Main.sdf.parse(startTime), Main.sdf.parse(endTime));
