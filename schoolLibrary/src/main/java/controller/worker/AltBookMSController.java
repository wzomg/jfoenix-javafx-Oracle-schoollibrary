...
public class AltBookMSController {
     ...
     */
    @FXML
    public void altSureClick() throws ParseException {
        //书号不能编辑
        ...
        int cnt = template.update(sql, bName, bAuthor, bkindId, Double.valueOf(bPrice), Main.sdf.parse(bDate.toString()), bPress, bSummary, bid);
        if(cnt > 0) {
    ...
    public void bSelectClick() {
        ...
        alDatePicker.getEditor().setText((bookLen.get(0).get("BDate").toString()).split(" ")[0]);
