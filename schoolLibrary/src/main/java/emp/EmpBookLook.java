...
public class EmpBookLook {
    ...
    public EmpBookLook(Book book) {
        ...
        if(book.getBPrice() != null) this.BPrice.set(book.getBPrice());
        if(book.getBStatus() != null) this.BStatus.set(book.getBStatus() == 1 ? "可借" : "已借出");
        this.TName.set(book.getTName());
        if(book.getBCnt() != null) this.BCnt.set(book.getBCnt());
        if(book.getNBTime() != null) this.NBTime.set(String.valueOf(book.getNBTime()).split(" ")[0]);
        if(book.getNBRTime() != null) this.NBRTime.set(String.valueOf(book.getNBRTime()).split(" ")[0]);
        ...
        if(book.getHBTime() != null) this.HBTime.set(String.valueOf(book.getHBTime()).split(" ")[0]);
        if(book.getHBRTime() != null) this.HBRTime.set(String.valueOf(book.getHBRTime()).split(" ")[0]);
        if(book.getQSDate() != null) this.QSDate.set(String.valueOf(book.getQSDate()).split(" ")[0]);
        if(book.getQEDate() != null) this.QEDate.set(String.valueOf(book.getQEDate()).split(" ")[0]);
        Integer qs = book.getQState();
        if(qs != null) { // 状态值为2表示可以借阅
            this.QState.set(qs == 0 ? "等待处理" : (qs == 1 ? "预约成功" : "可以借阅"));
        }
        if(book.getUUId() != null) this.UId.set(book.getUUId());
        if(book.getUSDate() != null)this.USDate.set(book.getUSDate().split(" ")[0]);
        if(book.getUEDate() != null) this.UEDate.set(book.getUEDate().split(" ")[0]);
